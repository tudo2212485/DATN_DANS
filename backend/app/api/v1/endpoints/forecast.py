...
from app.services.job_service import active_model
...
def read_forecast(
    ...
    model_name: Optional[str] = Query(None, description="Mô hình (LSTM, Prophet, ARIMA)"),
    days: int = Query(10, ge=1, le=30, description="Số ngày dự báo sau mốc lịch sử cuối (tối đa 30)"),
    db: Session = Depends(get_db)
):
    """Lấy dữ liệu dự báo giá, dải khoảng tin cậy 95% và các chỉ số sai số (MAE, RMSE, MAPE, R2)"""
    return get_forecast_dashboard(db, commodity_id=commodity_id, model_name=model_name or active_model(db), days=days)
